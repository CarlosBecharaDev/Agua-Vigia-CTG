...
| RF001–RF004 | func | M1/M5/M8: `SECTORES_MOCK`/`MOCK_EVENTOS` retirados de `useDatosEnVivo.ts`, `PaginaVeedor.tsx` y `PaginaBitacora.tsx` — cierra `DT-001`–`DT-005`. De paso, `BUG-032` (S1) encontrado y cerrado: `ListaSectores.tsx` mostraba un conteo de "reportes ciudadanos" inventado, siempre visible. Escrito y fusionado por D5 (Yordy) directo — capa de D4 (José), decisión explícita para no atrasar más el Sprint 1 | D5 (Yordy), en capa de D4 | [#86](https://github.com/CarlosBecharaDev/Agua-Vigia-CTG/pull/86) | `npm run build` y `npm test` → **6/6 pruebas en verde** |
