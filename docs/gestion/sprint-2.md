...
| D4 (José) | RF008 | ✅ Entregado — `FormularioReporte` consume `POST /api/reportes`, genera una huella anónima SHA-256 estable, permite coordenada opcional y solo confirma éxito después del `201` real. Sin fallback ni datos simulados; [PR #129](https://github.com/CarlosBecharaDev/Agua-Vigia-CTG/pull/129) fusionado a `develop` | `POST /api/reportes` ✅ (D3, este sprint) |
