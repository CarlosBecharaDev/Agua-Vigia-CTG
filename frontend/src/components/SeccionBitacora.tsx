...
import { obtenerBoletinesRecientes, determinarEstadoBoletin } from '../api/acuacar'
...
import { RefreshCw, ExternalLink, CheckCircle2, AlertTriangle, Info } from 'lucide-react'

// MOCK DATA — fallback si la API de Acuacar no responde
const MOCK_EVENTOS = [
  { id: 'ev-001', fecha: new Date(Date.now() - 10 * 60000).toISOString(), titulo: 'MANGA — corte confirmado por reporte de vecinos (5 reportes)', estado: 'SIN_SERVICIO' as const },
  { id: 'ev-002', fecha: new Date(Date.now() - 2 * 3600000).toISOString(), titulo: 'BOCAGRANDE — mantenimiento preventivo en tubería principal', estado: 'CORTE_PROGRAMADO' as const },
  { id: 'ev-003', fecha: new Date(Date.now() - 24 * 3600000).toISOString(), titulo: 'GETSEMANÍ — servicio reanudado oficialmente', estado: 'CON_SERVICIO' as const },
]
...
export const SeccionBitacora: FC<Props> = ({ busqueda = '' }) => {
  ...
  const cargarBoletines = useCallback(async () => {
    setCargando(true)
    try {
      const barriosCartagena = await obtenerBarriosCartagena().catch(() => undefined)
      const datos = await obtenerBoletinesRecientes(20, barriosCartagena)
      if (datos.length > 0) {
        setItems(datos.map((b) => ({
          id: String(b.id),
          numero: b.numero,
          titulo: b.titulo.replace(/^#\d+\s*–?\s*/, ''),
          fecha: b.fecha,
          estado: determinarEstadoBoletin(b.titulo),
          urlOficial: b.url ?? null,
          imagenUrl: b.imagenUrl,
          imagenAlt: b.imagenAlt,
        })))
        setUsandoDatosReales(true)
      } else {
        setItems(MOCK_EVENTOS.map((e) => ({ id: e.id, numero: null, titulo: e.titulo, fecha: e.fecha, estado: e.estado, urlOficial: null, imagenUrl: null, imagenAlt: '' })))
      ...
      <div className="bitacora-envoltorio">
        <div className="bitacora-cab">
          <div>
            ...
            <h2 className="bitacora-titulo">Últimos boletines de Acuacar</h2>
            <p className="bitacora-subtitulo">
              Registro público de mantenimientos, suspensiones y restablecimientos del servicio en Cartagena.
            </p>
          </div>
          <button onClick={cargarBoletines} disabled={cargando} className="bitacora-actualizar hover-glowing">
        ...
        <div className="bitacora-filtros" role="tablist" aria-label="Filtrar bitácora por estado">
          ...
          <span className="bitacora-fuente">
            {cargando ? 'conectando con acuacar…' : usandoDatosReales ? 'datos en vivo' : 'modo demo, sin conexión'}
          </span>
        </div>
        ...
        {itemsFiltrados.length === 0 && !cargando ? (
          <p className="bitacora-vacio">
            {busqueda.trim()
              ? `No hay boletines que coincidan con "${busqueda.trim()}".`
              : 'No hay boletines para este filtro todavía.'}
          </p>
        ) : (
          <div
            ref={carruselRef}
              ...
              const contenido = (
                <>
                  <div
                    ...
                    style={!item.imagenUrl ? { background: `color-mix(in srgb, ${color} 16%, var(--color-fondo))` } : undefined}
                  >
                    {item.imagenUrl ? (
                      <>
                        <img
                          ...
                          className="bitacora-tarjeta-imagen"
                          onError={(e) => { e.currentTarget.style.display = 'none' }}
                        />
                        <span className="bitacora-tarjeta-imagen-velo" aria-hidden="true" />
                        <span className="bitacora-tarjeta-insignia" style={{ '--color-insignia': color } as CSSProperties}>
                    ...
                    {item.numero && <span className="bitacora-tarjeta-numero">{item.numero}</span>}
                  ...
                  <div className="bitacora-tarjeta-pie">
                    ...
                    <h3>{item.titulo}</h3>
                    {item.urlOficial && (
                      <span className="bitacora-tarjeta-boton">
                        <ExternalLink size={13} aria-hidden="true" /> Leer documento
                      </span>
                    )}
                  </div>
                </>
              )
              return item.urlOficial ? (
                <a
                  key={`${item.id}-${i}`}
