/**
 * StrokeText — de reactbits.dev (Text Animations → Stroke Text), sin modificar.
 * Dibuja el contorno de cada letra y después le entra el relleno con un barrido.
 *
 * Se usa en PanelProyecto para el titular "AGUA VIGÍA": son DOS instancias, una por
 * palabra, porque el componente aplica un solo `strokeColor`/`fillColor` a todo el texto
 * y las dos palabras van de distinto color.
 *
 * Requiere `gsap` (core + ScrollTrigger), instalado como dependencia para esto.
 */
...
export interface StrokeTextProps {
  ...
  style?: CSSProperties;
  /**
   * Solo con trigger="loop": segundos entre el inicio de un dibujo y el siguiente.
   * El original deja una pausa fija de 0.9s tras cada pasada, lo que ata el ciclo a la
   * duración de la animación. Acá se pide el ciclo completo y la pausa se despeja restando
   * la duración real del timeline, así el intervalo se respeta aunque cambien los tiempos.
   */
  loopEvery?: number;
}
...
const StrokeText = ({
  ...
  useEffect(() => {
    ...
    const build = () => {
      ...
      // Se calcula después de armar los tweens porque hasta acá no se conoce la duración
      // real del timeline (depende del stagger y del número de letras).
      if (trigger === 'loop' && loopEvery) {
        tl.repeatDelay(Math.max(0, loopEvery - tl.duration()));
      }
  ...
  }, [box, dash, drawDuration, fillDelay, stagger, ease, trigger, fillMode, reverse, loopEvery]);
