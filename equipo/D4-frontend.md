...
> **Módulos asignados:** M1 (Mapa en vivo), M2 (Reporte ciudadano), M5 (Panel del veedor), M8 (Bitácora pública).
...
- **Genera el cliente HTTP tipado** a partir de la especificación OpenAPI publicada por D3.
...
| **Sprint 3** | • Interfaz del Panel del Veedor: Carga de cortes oficiales, registro de hora real y moderación.<br>• Formulario de suscripción a alertas por correo con doble opt-in.<br>• Pantalla de desuscripción de 1-clic. |
| **Sprint 4** | • **Visualización del Índice de Cumplimiento**: Barras comparativas (*Duración Prometida vs. Duración Real*).<br>• Gráficas estadisticas con Recharts.<br>• Vista de Bitácora Pública (timeline inmutable). |
...
- **Generación de Cliente HTTP**: Usar `openapi-typescript` o `orval` para sincronizar los tipos de endpoints directamente desde la especificación backend de D3.
